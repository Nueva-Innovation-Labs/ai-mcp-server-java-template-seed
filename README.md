...
| **4** | **HTTP Request (MCP Server)** | An **HTTP Request Node** (or a dedicated n8n Spring AI node) is executed. It makes a `POST` request to the MCP Server endpoint: `http://localhost:8080/mcp`. The body contains the tool call generated by the AI. |
